...
import { compressLogoForTitlePage, compressLogoForHeader, formatFileSize, } from '../utils/imageCompression.js';
export class PDFGenerator {
    browser = null;
    async generate(html, headings, config) {
        try {
            // Launch browser
            this.browser = await chromium.launch({ headless: true });
            // Step 1: Compress logos
            logger.step(0, 4, 'Compressing logos for optimal file size...');
            const titleLogo = await compressLogoForTitlePage(config.branding.logo.titlePath, config.branding.logo.titlePageWidth, 21 * 1024);
            const headerLogo = await compressLogoForHeader(config.branding.logo.headerPath, config.branding.logo.headerHeight, 1024);
            ...
            const contentPdfPath = await this.generateContent(html, extraction.headings, config, headerLogo.path);
            ...
            const finalPdfPath = await this.mergePDFs([titlePdfPath, contentPdfPath], config.output);
            // Get file stats
            ...
            logger.success(`Generated PDF: ${path.basename(finalPdfPath)} (${formatFileSize(result.fileSize)})`);
            return result;
        }
        finally {
    ...
    async generateTitlePage(config, logoPath) {
...
<body>
    ...
    async extractPageNumbers(html, headings, config, logoPath) {
        const page = await this.browser.newPage();
        ...
        const positions = await page.evaluate((headingIds) => {
            const results = {};
    ...
    async generateContent(html, headings, config, logoPath) {
        const page = await this.browser.newPage();
    ...
    buildContentHTML(content, headings, config, _logoPath, includeTOC) {
...
<body>
    ...
    buildHeaderTemplate(config, logoPath) {
        const logoBase64 = fs.readFileSync(logoPath).toString('base64');
    ...
    async mergePDFs(sources, output) {
     ...
     */
    async validateLayout(html, config, _logoPath) {
        const page = await this.browser.newPage();
        await page.setViewportSize({ width: 816, height: 1056 });
