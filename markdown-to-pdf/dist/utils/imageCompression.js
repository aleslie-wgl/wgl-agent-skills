 ...
 */
export async function compressLogoForTitlePage(logoPath, targetWidth, targetSize) {
    const originalSize = fs.statSync(logoPath).size;
    const tempDir = path.join(os.tmpdir(), 'pdf-logos');
    if (!fs.existsSync(tempDir)) {
        fs.mkdirSync(tempDir, { recursive: true });
    }
    const outputPath = path.join(tempDir, `title-logo-${Date.now()}.png`);
 ...
 * @returns Path to compressed logo
 */
export async function compressLogoForHeader(logoPath, targetHeight, targetSize) {
    const originalSize = fs.statSync(logoPath).size;
    const tempDir = path.join(os.tmpdir(), 'pdf-logos');
    if (!fs.existsSync(tempDir)) {
        fs.mkdirSync(tempDir, { recursive: true });
    }
    const outputPath = path.join(tempDir, `header-logo-${Date.now()}.png`);
    // Start with high quality
    let quality = 90;
    let compressedSize = originalSize;
    // Iteratively reduce quality until we hit target size
